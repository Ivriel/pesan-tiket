            ...
            <div
                ...
                <div class="relative h-64 w-full">
                    <img src="{{ asset('storage/' . $schedule->transportation->image) }}"
                        alt="{{ $schedule->transportation->name }}" class="w-full h-full object-cover">
                ...
                <div class="p-8">
                    <div
                        class="flex flex-col md:flex-row justify-between items-center mb-10 pb-8 border-b border-gray-100 dark:border-gray-700">
                        <div class="text-center md:text-left">
                            <p class="text-sm text-gray-500 dark:text-gray-400 uppercase tracking-widest font-semibold">
                                Keberangkatan</p>
                            <h3 class="text-2xl font-bold text-gray-900 dark:text-white">
                                {{ $schedule->route->departure }}</h3>
                            <p class="text-sm text-blue-600 font-medium">
                                {{ \Carbon\Carbon::parse($schedule->date_departure)->format('d M Y, H:i') }}</p>
                        </div>

                        <div class="flex flex-col items-center py-4 md:py-0">
                            <div
                                class="w-12 h-12 bg-blue-50 dark:bg-blue-900/30 rounded-full flex items-center justify-center">
                        ...
                        <div class="text-center md:text-right">
                            <p class="text-sm text-gray-500 dark:text-gray-400 uppercase tracking-widest font-semibold">
                                Tujuan</p>
                            <h3 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $schedule->route->arrival }}
                            </h3>
                            <p class="text-sm text-blue-600 font-medium">
                                {{ \Carbon\Carbon::parse($schedule->date_arrival)->format('d M Y, H:i') }}</p>
                        </div>
                    </div>
                    ...
                    <form action="{{ route('bookings.store') }}" method="POST"> @csrf
                        ...
                        <div class="mb-8">
                            ...
                            <select name="type_id" id="type_id"
                                class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:ring-blue-500 focus:border-blue-500 shadow-sm transition"
                                required>
                            ...
                            <div id="passengerContainer">
                                <div class="passenger-form bg-gray-50 dark:bg-gray-700 p-4 rounded-lg mb-4">
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <div>
                                            <label
                                                class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Nama
                                                Lengkap</label>
                                        ...
                                        <div>
                                            <label
                                                ...
                                                Telepon</label>
                                            <input type="text" name="passengers[0][phone]"
                                                class="w-full rounded border-gray-300 dark:border-gray-600 dark:bg-gray-800 text-sm"
                                                required>
                                        </div>
                                        <div>
                                            <label
                                                class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Kursi</label>
                                            <select name="passengers[0][seat_number]"
                                                class="seat-select w-full rounded border-gray-300 dark:border-gray-600 dark:bg-gray-800 text-sm"
                                                required>
                                    ...
                                    <button type="button"
                                        class="remove-passenger text-red-600 hover:text-red-700 text-xs mt-2 hidden">Hapus
                                        Penumpang</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Submit Button -->
                        <button type="submit"
                            class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-lg transition duration-200 transform hover:scale-[1.02] active:scale-95 shadow-lg shadow-blue-500/30">
                            Konfirmasi Pemesanan
                        </button>
                    </form>

                    <p class="mt-6 text-xs text-center text-gray-400">
                        ID Jadwal: #{{ $schedule->id }} • Pastikan data keberangkatan sudah sesuai sebelum melakukan
                        pemesanan.
                    </p>
                </div>
            </div>
