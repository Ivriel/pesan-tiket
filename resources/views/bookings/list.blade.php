                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                <thead class="bg-gray-50 dark:bg-gray-900/50">
                                    <tr>
                                        ...
                                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Pelanggan</th>
                                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Rute & Kelas</th>
                                    ...
                                    @foreach($bookings as $booking)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                ...
                                                <div class="text-xs text-gray-400">{{ $booking->created_at->format('d M Y, H:i') }}</div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm font-medium">{{ $booking->schedule->route->departure }} → {{ $booking->schedule->route->arrival }}</div>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div class="flex justify-end gap-2">
                                                    ...
                                                    @if($booking->status === 'pending' && auth()->id() == $booking->user_id)
                                                        <form action="{{ route('bookings.pay', $booking->id) }}" method="POST" title="Bayar Sekarang">
                                                            @csrf @method('PATCH')
                                                            <button class="p-2 bg-green-500 hover:bg-green-600 text-white rounded-lg transition shadow-sm">
                                                        ...
                                                        <form action="{{ route('bookings.cancel', $booking->id) }}" method="POST" onsubmit="return confirm('Batalkan pesanan ini?')" title="Batalkan">
                                                            @csrf @method('PATCH')
                                                    ...
                                                    {{-- Tombol Detail --}}
                                                    <a href="{{ route('bookings.show', $booking->id) }}" class="p-2 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-lg transition" title="Lihat Detail">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
